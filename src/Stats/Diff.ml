...
let create isSimilar isEqual xs ys =
  ...
  and (intact, modified) = (Utils.List.intersect isSimilar xs ys)
    |> List.map (fun z ->
      let findSimilar = List.find (isSimilar z)
      in (findSimilar xs, findSimilar ys)
    )
    |> List.partition (fun (x, y) -> isEqual x y)
  in {
    added = diff ys xs;
    intact = intact |> List.map fst;
    modified = modified;
    removed = diff xs ys;
  };;
