...
let make = (~modules, ~onModule, _children) => {
  let renderModulesDiff = (mapper, title, onModule, modules) =>
    <ModulesDiff modules={modules |> List.map(mapper)} title onModule />;
  {
    ...component,
    render: _self => {
      let added =
        renderModulesDiff(
          ...
          L10N.added,
          module_ => onModule(Segment.of_module(Added, module_)),
          modules.added,
        );
      let removed =
        renderModulesDiff(
          ...
          L10N.removed,
          module_ => onModule(Segment.of_module(Removed, module_)),
          modules.removed,
        );
      let intact =
        renderModulesDiff(
          ...
          L10N.intact,
          module_ => onModule(Segment.of_module(Intact, module_)),
          modules.intact,
        );
      let modified =
        renderModulesDiff(
          ...
          L10N.modified,
          module_ => onModule(Segment.of_module(Modified, module_)),
          modules.modified,
        );
      <>
