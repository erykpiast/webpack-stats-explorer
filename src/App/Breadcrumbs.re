module Styles = {
  ...
  let wrapper =
    style([display(`flex), alignItems(`center), width(`percent(100.0))]);
  ...
  module Badge = {
    ...
    let added = style([backgroundColor(Theme.worseColor), ...badgeRules]);

    let intact = style(badgeRules);

    let modified =
      style([backgroundColor(Theme.changeColor), ...badgeRules]);

    let removed = style([backgroundColor(Theme.betterColor), ...badgeRules]);
  };
};
...
let make = (~items, ~onClick, _children) => {
  ...
  render: _self => {
    let head =
      <li className=Styles.item onClick={_ => onClick(0)}>
    ...
    let separator =
      <li className=Styles.separator> {">" |> ReasonReact.string} </li>;
    let breadcrumbs = [head, ...tail] |> Utils.List.join(separator);

    <ul className=Styles.wrapper> ...{breadcrumbs |> Array.of_list} </ul>;
  },
};